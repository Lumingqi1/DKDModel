...
class ModelOptimizer:
    ...
    def optimize_hyperparams(self, C, epsilon, gamma):
        params = {
            'C': max(0.1, C),
        ...
        # Create and train the Model3_SVR model
        model = SVR(
            kernel='rbf',
            C=params['C'],
...
def ModelSVR(train, val, test, type):
    ...
    pd.DataFrame([optimizer_params]).to_excel(
        f'./output_params/Model3_SVR_{type}.xlsx',
        index=False
    )

    # Save model
    os.makedirs('./output_model', exist_ok=True)
    dump(best_model, f'./output_model/Model3_SVR_{type}.joblib')
